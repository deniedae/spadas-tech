...
export default function LensCompsModal({
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const { error } = await createListing({
        userId: user.id,
        product: title,
        description: `Sourced via Spadas Lens AR. Category: ${category}. Condition: ${condition}. True Net Profit: +$${netProfit} AUD (${roi}% ROI). Verified sold comps comps source: eBay AU.`,
        price: estValue,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/85 backdrop-blur-md animate-fade-in touch-none">
      <div className="relative w-full max-w-lg h-[92vh] sm:h-auto sm:max-h-[90vh] flex flex-col rounded-t-3xl sm:rounded-3xl bg-slate-900 border border-cyan-500/40 shadow-[0_0_50px_rgba(6,182,212,0.25)] text-slate-100 overflow-hidden animate-slide-up">
        
        {/* ── 1. Top Fixed Header ───────────────────────────────────────────── */}
        <div className="flex shrink-0 items-center justify-between border-b border-slate-800 p-4 sm:p-5 bg-slate-950/90 z-10">
        ...
        {/* ── 2. Scrollable Body Content (Smooth Touch Scroll) ───────────────── */}
        <div className="flex-1 overflow-y-auto overscroll-contain p-4 sm:p-5 space-y-4 touch-pan-y no-scrollbar">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 border border-slate-800 shadow-xl space-y-3">
            <div className="flex items-center justify-between">
              <div>
              ...
              <div className="text-right">
                <span
                  className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-black border shadow-lg ${
                    copVerdict === "MUST_COP"
            ...
            <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-800/80 text-center">
              ...
              <div className="p-2 rounded-xl bg-slate-950/80 border border-slate-800/60">
                <span className="text-[9px] uppercase font-bold text-slate-400 block">eBay AU Fees</span>
        ...
        {/* ── 3. Fixed Bottom Action Bar (Always Visible & Accessible) ──────── */}
        <div className="shrink-0 p-4 sm:p-5 border-t border-slate-800 bg-slate-950/95 backdrop-blur-md space-y-2.5 z-10">
          {/* Main Hero: Scan Next Item */}
          <button
            type="button"
            onClick={onResumeScan}
            className="w-full flex items-center justify-center gap-2.5 rounded-2xl bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 py-3 text-sm font-black text-white shadow-[0_0_30px_rgba(6,182,212,0.4)] hover:opacity-95 active:scale-[0.98] transition cursor-pointer"
          >
