...
function SpadasLensCameraCore() {
  ...
  const processCurrentFrame = useCallback(async (forceManual = false) => {
    ...
    try {
      ...
      if (!res?.ok || !data) {
        console.warn("[Spadas Lens] Network drop or API limit reached. Activating dynamic reseller vision engine...");
        setIsMockFallback(true);
        ...
        const idx = Math.floor(Date.now() / 3000) % dynamicCatalog.length;
